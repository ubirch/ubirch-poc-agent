...
class ScalatraBootstrap extends LifeCycle with LazyLogging {
  ...
  lazy val allowedCorsHeaders: String = (CorsConfig.DefaultAllowedHeaders ++ config.getString(HttpServerConfPaths.GENERAL_ADDITIONAL_CORS_HEADERS).split(",").toSeq.filter(_.nonEmpty)).mkString(",")
  ...
  override def init(context: ServletContext): Unit = {
    logger.info(s"swagger_on_path=$swaggerOnPath allowed_cors_headers=($allowedCorsHeaders) allowed_methods=(${CorsConfig.DefaultMethods})")
